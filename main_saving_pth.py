...
def parse_arguments():
    ...
    # parser.add_argument('--video-path', default='/fsx/marijajegorova/pre-process/step2_affine_transformation/RECOLA_split_140/', help='video data path')
    # parser.add_argument('--annot-path', default='/fsx/marijajegorova/pre-process/step2_affine_transformation/RECOLA_annot_20secs_binned_new/', help='annotation path')
    ...
    parser.add_argument('--phase', type = str, default = 'train', choices = ['train','test'], help = 'Are we training or testing?' )
...
def get_model(args, device): # defines the model architecture
    model = EmotionModelVideo(fine_tuning=args.fine_tuning, bins=args.bins, n_classes=args.n_classes, device=device, every_frame = args.every_frame).to(device)
    return model
...
def test(model, dset_loader, logger, phase):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(dset_loader):
            ...
            inputs = data['video']
            if phase == 'test':
                lengths = tuple([inputs.size(1)] * args.len_tar)
                inputs = inputs.to(device)
                outputs = model(inputs, lengths)
                out = outputs.cpu().numpy()
                filenames = data['paths'][0]
                count = 0
                for i, f in enumerate(filenames):
                    names = f.split(os.sep)[-1]
                    count +=1
                    np.savez('./Custom_outs/' + names, out[i, :, :]) 
                # print('COUNT: ', count)

            else:
                targets = get_targets(data, args.output_type)
                args.len_tar = len(targets)
    ...
    while epoch < args.epochs:
        if ( get_lr(optimizer) > args.min_lr ):
            ...
            if current_perf > best_perf:
                ...
                best_epoch = epoch
                save_params_path = os.path.join(save_path, 'bestParams.pth')
                torch.save(save_dict, save_params_path)
